...
class GPPCommand(CommandProgram):
    """
    Command to verify if ``g++`` command is recognized by the operating system.
    ...
    def __init__(self, allow_install: bool, update_package_manager: bool = True) -> NoReturn:
        ...
        windows = WindowsInstallationPackage(
            ...
            choco_command="choco install gcc",
        ...
        linux = GNULinuxDistributionInstallationPackage(
            ...
            dnf_command="sudo dnf install gcc-c++",
            yum_command="sudo yum install gcc-c++",
            pacman_command="sudo pacman -S gcc",
            update_package_manager=update_package_manager
        )
        super().__init__("g++ --version", allow_install,
                         windows, macos, linux)
